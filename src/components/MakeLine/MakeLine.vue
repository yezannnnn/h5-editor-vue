<template>
  <div class="mark-line">
    <div v-for="line in lines" :key="line" class="line" :class="line.includes('x')? 'xline' : 'yline'" :ref="line" v-show="lineStatus[line] || false"></div>
  </div>
</template>
<script>
...
export default {
  ...
  methods: {
    ...
    showLine(isDownward, isRightward) {
      ...
      const components = this.sourceData;
      const curComponentStyle = getComponentRotatedStyle(this.curComponent);
      ...
      components.forEach((component) => {
        ...
        const needToShow = [];
        const { rotate } = this.curComponent;
        Object.keys(conditions).forEach((key) => {
          // 遍历符合的条件并处理
          conditions[key].forEach((condition) => {
            if (!condition.isNearly) return;
            // 修改当前组件位移
            this.$store.commit('components/setShapeSingleStyle', {
              key,
    ...
    translatecurComponentShift(key, condition, curComponentStyle) {
      const { width, height } = this.curComponent;
      if (key === 'top') {
